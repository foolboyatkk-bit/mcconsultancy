...
const OFFICE_DATA = [
  {
    ...
    phones: ["+91 9500700113", "+91 8903492113"],
  ...
  {
    tag: "Branch Office",
    title: "Thillai Nagar, Trichy",
    address: "5th Cross, Thillai Nagar, Trichy, Tamil Nadu — 629501",
    email: "mandc.edu2020@gmail.com",
    phones: ["+91 9500700113", "+91 8903492113"],
    landline: "04651-272113",
  },
...
const GLOBAL_STYLES = `
  ...
  .ci-card-2  { animation: ci-fade-up 0.55s ease both; animation-delay: 0.41s; }
  ...
  /* ── SUBGRID: aligns ADDRESS / EMAIL / MOBILE / LANDLINE rows across all 3 cards ── */
  @media (min-width: 1024px) {
    .ci-cards-grid {
      grid-template-rows: auto auto auto auto auto; /* header + 4 info rows */
      row-gap: 0 !important;                        /* rows inside a card must not gap */
    }
    .ci-card {
      grid-row: span 5;             /* each card spans all 5 shared row tracks */
      display: grid;
      grid-template-rows: subgrid;  /* adopt parent's row sizing */
    }
    .ci-card-body {
      display: contents; /* dissolves wrapper so InfoRows become direct subgrid items */
    }
    /* Restore horizontal padding lost when ci-card-body becomes display:contents */
    .ci-card-body .ci-info-row {
      padding-left: 2rem;
      padding-right: 2rem;
    }
    /* Restore card top padding on first row (overrides the base padding-top:0 rule) */
    .ci-card-body .ci-info-row:first-child {
      padding-top: 2rem !important;
    }
    /* Restore card bottom padding on last row */
    .ci-card-body .ci-info-row:last-child {
      padding-bottom: 2rem;
    }
  }
...
const STYLES: Record<string, React.CSSProperties> = {
  ...
  tagWrap: {
    ...
    textTransform: "uppercase", color: "#1976d2", backgroundColor: "#eef3fd",
    border: "1px solid #c7d8f5", padding: "6px 16px", borderRadius: "2px", marginBottom: "16px",
  },
  tagDot: { width: 6, height: 6, borderRadius: "50%", backgroundColor: "#1976d2", display: "inline-block" },
  heading: {
    fontFamily: "'Inter', sans-serif",
    fontSize: "clamp(1.75rem, 4vw, 2.5rem)",
    fontWeight: 700, color: "#0f172a", margin: "0 0 16px", lineHeight: 1.2,
  },
  gradientText: {
    color: "transparent",
  ...
  cardHeader: {
    background: "linear-gradient(110deg, #0f2265 0%, #1a3a8f 60%, #1565c0 100%)",
    display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px",
  },
  cardTag: {
    display: "inline-block", fontSize: "0.6rem", fontWeight: 500,
    letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(147,180,232,0.8)", marginBottom: "6px",
  },
  cardTitle: { fontSize: "1.2rem", fontWeight: 700, color: "#ffffff", margin: 0, lineHeight: 1.3 },
  badge: {
    flexShrink: 0, width: "36px", height: "36px", borderRadius: "50%",
    border: "1.5px solid rgba(147,180,232,0.3)", display: "flex", alignItems: "center", justifyContent: "center",
    fontFamily: "'Playfair Display', serif", fontSize: "1rem", fontWeight: 700, color: "rgba(219,233,255,0.7)",
  },
  rowLabel: {
    display: "block", fontWeight: 500, letterSpacing: "0.1em",
    textTransform: "uppercase", color: "#1976d2", marginBottom: "4px",
  },
    ...
    <>
      ...
      <section style={STYLES.section} className="py-16 px-5 sm:py-24 sm:px-12">
        <div style={STYLES.container} className="w-full">
          ...
          {/* ── Office cards grid — "ci-cards-grid" enables subgrid alignment ── */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 ci-cards-grid">
            {OFFICE_DATA.map((office, index) => (
              <div key={index} className={`ci-card ci-card-${index}`}>

                {/* Card header — row 1 of the subgrid */}
                <div style={STYLES.cardHeader} className="p-6 sm:p-8">
                  ...
                  <span style={STYLES.badge}>{String(index + 1).padStart(2, "0")}</span>
                </div>

                {/* Card body — "ci-card-body" dissolves at lg so each InfoRow
                    becomes a direct subgrid item (rows 2-5) */}
                <div className="ci-card-body p-6 sm:p-8">
                  <InfoRow icon={<PinIcon />} label="Address">
                    <p style={STYLES.rowText}>{office.address}</p>
                  </InfoRow>

                  <InfoRow icon={<MailIcon />} label="Email">
                    <a href={`mailto:${office.email}`} className="ci-link" style={STYLES.rowLink}>
                      {office.email}
                    </a>
                  </InfoRow>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                      {office.phones.map((num, idx) => (
                        <React.Fragment key={num}>
                          <a href={`tel:${num}`} className="ci-link" style={STYLES.rowLink}>{num}</a>
                          {idx !== office.phones.length - 1 && (
                            <span className="text-[#374151] text-[0.82rem]">,</span>
                          )}
                        </React.Fragment>
                      ))}
                    </div>
                  ...
                  <InfoRow icon={<LandlineIcon />} label="Landline">
                    <a href={`tel:${office.landline}`} className="ci-link" style={STYLES.rowLink}>
                      {office.landline}
                    </a>
                  </InfoRow>
                </div>
